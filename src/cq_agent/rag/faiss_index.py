...
# Check for sentence_transformers availability without importing it
...
try:
    ...
    if spec is not None:
        ...
        print("[FAISS] sentence-transformers found but not loaded yet")
    else:
        print("[FAISS] sentence-transformers not found")
except Exception as e:
    print(f"[FAISS] Error checking sentence_transformers: {e}")

# Create dummy classes if dependencies not available
if not (FAISS_AVAILABLE and SENTENCE_TRANSFORMERS_AVAILABLE):
    print("[FAISS] Optional dependencies not available. Install with: pip install faiss-cpu sentence-transformers")
    
    class FAISSRAGIndex:
        def __init__(self, *args, **kwargs):
            raise ImportError("FAISS dependencies not available")
    
    def create_faiss_rag_index(*args, **kwargs):
        return None
    ...
    # Real implementations only when dependencies are present
    ...
    class FAISSRAGIndex:
        ...
        def __init__(self, model_name: str = "all-MiniLM-L6-v2", dimension: int = 384):
            ...
            try:
                ...
                raise ImportError("sentence-transformers import failed. Install with: pip install sentence-transformers") from e
